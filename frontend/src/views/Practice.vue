...
export default defineComponent({
  ...
  setup() {
    const state:any = reactive({
      ...
      font: 80,
      level: 1,
      title1: 'The Hare and the Tortoise',
      title2: 'Prince and the Princess',
      title3: 'Jack and the Beanstalk',
      title4: 'Goldilocks and the Three Bears',
      title5: 'Demo',
      completed: false,
    ...
    const setTextTitle = (max:number) => {
      let chosen:number = Math.floor(Math.random() * max) + 1
          state.text = state[`text${chosen}`] + "";
          state.title = state[`title${chosen}`] + "";
    }
    onMounted(() => {
          setTextTitle(5);
          onTextInput();
