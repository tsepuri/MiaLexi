<template>
  ...
  <div class="login">
    ...
    <div v-if="state.wrongInfo" id="wrong">
        ...
        <li>
        <a><router-link to="/create"><u>Don't have an account? Create one.</u></router-link></a>
        </li>
        <li>
        <a><router-link to="/home"><u>Continue without logging in.</u></router-link></a>
        </li>
  </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
      ...
      const checkLogin = () => {
          if (state.username === 'Tarun' && state.password === '12345') {
                store.commit('login');
                router.push('/'); 
          }
          else {
